		...
		_makeCode() {
			...
			if (!this._empty(this.val)) {
				...
				qrcode = new QRCode({
					...
					usingComponents: that.usingComponents, // 是否是自定義组件
					showLoading: false, // 是否顯示loading
					loadingText: that.loadingText, // loading文字
					text: that.val, // 生成内容
					size: that.size, // 二维碼大小
					background: that.background, // 背景色
					foreground: that.foreground, // 前景色
					pdground: that.pdground, // 定位角點顏色
					correctLevel: that.lv, // 容错級别
					image: that.icon, // 二维碼圖標
					imageSize: that.iconSize,// 二维碼圖標大小
					cbResult: function (res) { // 生成二维碼的回調
						that._result(res)
					},
				});
			} else {
