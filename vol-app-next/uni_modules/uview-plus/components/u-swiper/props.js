...
export const props = defineMixin({
    props: {
        // 列表數组，元素可為字符串，如為对象可通过keyName指定目標屬性名
        list: {
            type: Array,
        ...
        // 指示器樣式，可通过bottom，left，right进行定位
        indicatorStyle: {
            type: [String, Object],
        ...
        // 当前所在滑块的 item-id ，不能與 current 被同时指定
        currentItemId: {
            type: String,
        ...
        // 当开啟时，会根据滑动速度，连续滑动多屏，支付宝不支持
        acceleration: {
            type: Boolean,
            default: () => defProps.swiper.acceleration
        },
        // 同时显示的滑块數量，nvue、支付宝小程序不支持
        displayMultipleItems: {
            type: Number,
            default: () => defProps.swiper.displayMultipleItems
        },
        // 指定swiper切換缓动动画类型，有效值：default、linear、easeInCubic、easeOutCubic、easeInOutCubic
        // 只对微信小程序有效
        easingFunction: {
            type: String,
