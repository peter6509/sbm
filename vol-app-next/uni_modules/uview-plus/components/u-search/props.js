import { defineMixin } from '../../libs/vue'
import defProps from '../../libs/config/props.js'
export const props = defineMixin({
    props: {
        // 搜索框形状，round-圆形，square-方形
        shape: {
            type: String,
            default: () => defProps.search.shape
        },
        // 搜索框背景色，默认值#f2f2f2
        bgColor: {
            type: String,
            default: () => defProps.search.bgColor
        },
        // 占位提示文字
        placeholder: {
            type: String,
            default: () => defProps.search.placeholder
        },
        // 是否啟用清除控件
        clearabled: {
            type: Boolean,
            default: () => defProps.search.clearabled
        },
        // 是否自动聚焦
        focus: {
            type: Boolean,
            default: () => defProps.search.focus
        },
        // 是否在搜索框右侧显示取消按鈕
        showAction: {
            type: Boolean,
            default: () => defProps.search.showAction
        },
        // 右边控件的樣式
        actionStyle: {
            type: Object,
            default: () => defProps.search.actionStyle
        },
        // 取消按鈕文字
        actionText: {
            type: String,
            default: () => defProps.search.actionText
        },
        // 输入框内容对齊方式，可選值為 left|center|right
        inputAlign: {
            type: String,
            default: () => defProps.search.inputAlign
        },
        // input输入框的樣式，可以定义文字颜色，大小等，对象形式
        inputStyle: {
            type: Object,
            default: () => defProps.search.inputStyle
        },
        // 是否啟用输入框
        disabled: {
            type: Boolean,
            default: () => defProps.search.disabled
        },
        // 边框颜色
        borderColor: {
            type: String,
            default: () => defProps.search.borderColor
        },
        // 搜索图標的颜色，默认同输入框字体颜色
        searchIconColor: {
            type: String,
            default: () => defProps.search.searchIconColor
        },
        // 输入框字体颜色
        color: {
            type: String,
            default: () => defProps.search.color
        },
        // placeholder的颜色
        placeholderColor: {
            type: String,
            default: () => defProps.search.placeholderColor
        },
        // 左边输入框的图標，可以為uView图標名稱或图片路径
        searchIcon: {
            type: String,
            default: () => defProps.search.searchIcon
        },
        searchIconSize: {
            type: [Number, String],
            default: () => defProps.search.searchIconSize
        },
        // 组件與其他上下左右元素之间的距离，带單位的字符串形式，如"30px"、"30px 20px"等写法
        margin: {
            type: String,
            default: () => defProps.search.margin
        },
        // 开啟showAction时，是否在input获取焦点时才显示
        animation: {
            type: Boolean,
            default: () => defProps.search.animation
        },
        // 输入框的初始化内容
        modelValue: {
            type: String,
            default: () => defProps.search.value
        },
		value: {
		    type: String,
		    default: () => defProps.search.value
		},
        // 输入框最大能输入的長度，-1為不限制長度(来自uniapp文档)
        maxlength: {
            type: [String, Number],
            default: () => defProps.search.maxlength
        },
        // 搜索框高度，單位px
        height: {
            type: [String, Number],
            default: () => defProps.search.height
        },
        // 搜索框左侧文本
        label: {
            type: [String, Number, null],
            default: () => defProps.search.label
        },
        // 鍵盘彈起时，是否自动上推页面	
        adjustPosition: {
            type: Boolean,
            default: () => true
        },
        // 鍵盘收起时，是否自动失去焦点	
        autoBlur: {
            type: Boolean,
            default: () => false
        }
    }
})
