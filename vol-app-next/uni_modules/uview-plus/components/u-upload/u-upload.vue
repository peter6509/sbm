...
<script>
	...
	/**
	 * upload 上傳
	 * @description 該组件用于上傳圖片场景
	 * @tutorial https://uview-plus.jiangruyi.com/components/upload.html
	 * @property {String}			accept				接受的文件類型, 可選值為all media image file video （默認 'image' ）
	 * @property {String | Array}	capture				圖片或视频拾取模式，當accept為image類型時設置capture可選額外camera可以直接調起摄像頭（默認 ['album', 'camera'] ）
	 * @property {Boolean}			compressed			當accept為video時生效，是否压缩视频，默認為true（默認 true ）
	 * @property {String}			camera				當accept為video時生效，可選值為back或front（默認 'back' ）
	 * @property {Number}			maxDuration			當accept為video時生效，拍摄视频最長拍摄時間，單位秒（默認 60 ）
	 * @property {String}			uploadIcon			上傳區域的圖標，只能内置圖標（默認 'camera-fill' ）
	 * @property {String}			uploadIconColor		上傳區域的圖標的字體顏色，只能内置圖標（默認 #D3D4D6 ）
	 * @property {Boolean}			useBeforeRead		是否開啟文件讀取前事件（默認 false ）
	 * @property {Boolean}			previewFullImage	是否顯示组件自带的圖片預覽功能（默認 true ）
	 * @property {String | Number}	maxCount			最大上傳數量（默認 52 ）
	 * @property {Boolean}			disabled			是否啟用（默認 false ）
	 * @property {String}			imageMode			預覽上傳的圖片時的裁剪模式，和image组件mode屬性一致（默認 'aspectFill' ）
	 * @property {String}			name				標识符，可以在回調函數的第二項参數中獲取
	 * @property {Array}			sizeType			所選的圖片的尺寸, 可選值為original compressed（默認 ['original', 'compressed'] ）
	 * @property {Boolean}			multiple			是否開啟圖片多選，部分安卓機型不支持 （默認 false ）
	 * @property {Boolean}			deletable			是否展示删除按鈕（默認 true ）
	 * @property {String | Number}	maxSize				文件大小限制，單位為byte （默認 Number.MAX_VALUE ）
	 * @property {Array}			fileList			顯示已上傳的文件列表
	 * @property {String}			uploadText			上傳區域的提示文字
	 * @property {String | Number}	width				内部預覽圖片區域和選擇圖片按鈕的區域寬度（默認 80 ）
	 * @property {String | Number}	height				内部預覽圖片區域和選擇圖片按鈕的區域高度（默認 80 ）
	 * @property {Object}			customStyle			组件的樣式，對象形式
	 * @event {Function} afterRead		讀取后的處理函數
	 * @event {Function} beforeRead		讀取前的處理函數
	 * @event {Function} oversize		文件超出大小限制
	 * @event {Function} clickPreview	點擊預覽圖片
	 * @event {Function} delete 		删除圖片
			...
			formatFileList() {
				...
				const lists = fileList.map((item) =>
					Object.assign(Object.assign({}, item), {
						// 如果item.url為本地選擇的blob文件的話，無法判斷其為video還是image，此處优先通過accept做判斷處理
...
<style lang="scss" scoped>
	...
	.u-upload {
		...
		&__success {
			...
			@include flex;
			// 由于weex(nvue)為阿里巴巴的KPI(部门业绩考核)的laji產物，不支持css绘制三角形
			// 所以在nvue下使用圖片，非nvue下使用css實現
			/* #ifndef APP-NVUE */
