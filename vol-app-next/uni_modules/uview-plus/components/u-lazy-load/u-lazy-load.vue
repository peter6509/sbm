<template>
    <view class="u-wrap" :style="{
		opacity: Number(opacity),
		borderRadius: borderRadius + 'rpx',
		// 因為time值需要改變,所以不直接用duration值(不能改變父组件prop傳過来的值)
		transition: `opacity ${time / 1000}s ease-in-out`
	}" :class="'u-lazy-item-' + elIndex">
        <view :class="'u-lazy-item-' + elIndex">
            <image :style="{ borderRadius: borderRadius + 'rpx', height: imgHeight }" v-if="!isError"
                class="u-lazy-item" :src="isShow ? image : loadingImg" :mode="imgMode" @load="imgLoaded"
                @error="loadError" @tap="clickImg">
            </image>
            <image :style="{ borderRadius: borderRadius + 'rpx', height: imgHeight }" class="u-lazy-item error" v-else
                :src="errorImg" :mode="imgMode" @load="errorImgLoaded" @tap="clickImg"></image>
        </view>
    </view>
</template>

<script>
    import {
        addUnit,
        guid
    } from '../../libs/function/index.js';
    /**
     * lazyLoad 懒加載
     * @description 懒加載使用的场景為：頁面有很多圖片時，APP會同時加載所有的圖片，导致頁面卡顿，各個位置的圖片出現前后不一致等.
     * @tutorial https://uview-plus.jiangruyi.com/components/lazy-load.html
     * @property {String Number} index 用户自定義值，在事件触發時回調，用以區分是哪個圖片
     * @property {String} image 圖片路徑
     * @property {String} loading-img 預加載時的占位圖
     * @property {String} error-img 圖片加載出错時的占位圖
     * @property {String} threshold 触發加載時的位置，見上方說明，單位 rpx（默認300）
     * @property {String Number} duration 圖片加載成功時，淡入淡出時間，單位ms（默認）
     * @property {String} effect 圖片加載成功時，淡入淡出的css動画效果（默認ease-in-out）
     * @property {Boolean} is-effect 圖片加載成功時，是否啟用淡入淡出效果（默認true）
     * @property {String Number} border-radius 圖片圆角值，單位rpx（默認0）
     * @property {String Number} height 圖片高度，注意：實际高度可能受img-mode参數影响（默認450）
     * @property {String Number} mg-mode 圖片的裁剪模式，詳見image组件裁剪模式（默認widthFix）
     * @event {Function} click 點擊圖片時触發
     * @event {Function} load 圖片加載成功時触發
     * @event {Function} error 圖片加載失败時触發
     * @example <u-lazy-load :image="image" :loading-img="loadingImg" :error-img="errorImg"></u-lazy-load>
     */
    export default {
        name: 'u-lazy-load',
        props: {
            index: {
                type: [Number, String]
            },
            // 要顯示的圖片
            image: {
                type: String,
                default: ''
            },
            // 圖片裁剪模式
            imgMode: {
                type: String,
                default: 'widthFix'
            },
            // 占位圖片路徑
            loadingImg: {
                type: String,
                default: 'data:image/png;base64,'
            },
            // 加載失败的错误占位圖
            errorImg: {
                type: String,
                default: 'data:image/png;base64,'
            },
            // 圖片进入可見區域前多少像素時，單位rpx，開始加載圖片
            // 负數為圖片超出屏幕底部多少距离后触發懒加載，正數為圖片顶部距离屏幕底部多少距离時触發(圖片還没出現在屏幕上)
            threshold: {
                type: [Number, String],
                default: 100
            },
            // 淡入淡出動画的過渡時間
            duration: {
                type: [Number, String],
                default: 500
            },
            // 渡效果的速度曲线，各個之間差别不大，因為這是淡入淡出，且時間很短，不是那些變形或者移動的情况，會明顯
            // linear|ease|ease-in|ease-out|ease-in-out|cubic-bezier(n,n,n,n);
            effect: {
                type: String,
                default: 'ease-in-out'
            },
            // 是否使用過渡效果
            isEffect: {
                type: Boolean,
                default: true
            },
            // 圆角值
            borderRadius: {
                type: [Number, String],
                default: 0
            },
            // 圖片高度，單位rpx
            height: {
                type: [Number, String],
                default: '200'
            }
        },
        data() {
            return {
                isShow: false,
                opacity: 1,
                time: this.duration,
                loadStatus: '', // 默認是懒加載中的狀態
                isError: false, // 圖片加載失败
                elIndex: guid()
            }
        },
        computed: {
            // 將threshold從rpx轉為px
            getThreshold() {
                // 先取绝對值，因為threshold可能是负數，最后根據this.threshold是正數或者负數，重新還原
                let thresholdPx = uni.upx2px(Math.abs(this.threshold));
                return this.threshold < 0 ? -thresholdPx : thresholdPx;
            },
            // 計算圖片的高度，可能為auto，带%，或者直接數值
            imgHeight() {
                return addUnit(this.height);
            }
        },
        created() {
            // 由于一些特殊原因，不能將此變量放到data中定義
            this.observer = {};
        },
        watch: {
            isShow(nVal) {
                // 如果是不開啟過渡效果，直接返回
                if (!this.isEffect) return;
                this.time = 0;
                // 原来opacity為1(不透明，是為了顯示占位圖)，改成0(透明，意味着該元素顯示的是背景顏色，默認的白色)，再改成1，是為了獲得過渡效果
                this.opacity = 0;
                // 延時30ms，否则在浏覽器H5，過渡效果無效
                setTimeout(() => {
                    this.time = this.duration;
                    this.opacity = 1;
                }, 30)
            },
            // 圖片路徑發生變化時，需要重新標记一些變量，否则會一直卡在某一個狀態，比如isError
            image(n) {
                if (!n) {
                    // 如果傳入null或者''，或者undefined，標记為错误狀態
                    this.isError = true;
                } else {
                    this.init();
                    this.isError = false;
                }
            }
        },
        emits: ['click', 'load', 'error'],
        methods: {
            // 用于重新初始化
            init() {
                this.isError = false;
                this.loadStatus = '';
            },
            // 點擊圖片触發的事件,loadlazy-還是懒加載中狀態，loading-圖片正在加載，loaded-圖片加加載完成
            clickImg() {
                let whichImg = '';
                // 如果isShow為false，意味着圖片還没開始加載，點擊的只能是最開始的占位圖
                if (this.isShow == false) whichImg = 'lazyImg';
                // 如果isError為true，意味着圖片加載失败，這是只剩下错误的占位圖，所以點擊的只能是错误占位圖
                // 當然，也可以给错误的占位圖元素绑定點擊事件，看你喜欢~
                else if (this.isError == true) whichImg = 'errorImg';
                // 總共三張圖片，除了两個占位圖，剩下的只能是正常的那張圖片了
                else whichImg = 'realImg';
                // 只通知當前圖片的index
                this.$emit('click', this.index);
            },
            // 圖片加載完成事件，可能是加載占位圖時触發，也可能是加載真正的圖片完成時触發，通過isShow區分
            imgLoaded() {
                // 占位圖加載完成
                if (this.loadStatus == '') {
                    this.loadStatus = 'lazyed';
                }
                // 真正的圖片加載完成 
                else if (this.loadStatus == 'lazyed') {
                    this.loadStatus = 'loaded';
                    this.$emit('load', this.index);
                }
            },
            // 错误的圖片加載完成
            errorImgLoaded() {
                this.$emit('error', this.index);
            },
            // 圖片加載失败
            loadError() {
                this.isError = true;
            },
            disconnectObserver(observerName) {
                const observer = this[observerName];
                observer && observer.disconnect();
            },
        },
        beforeUnmount() {
            // 銷毁頁面時，可能還没触發某張很底部的懒加載圖片，所以把這個事件给去掉
            //observer.disconnect();
        },
        mounted() {
            // 此uOnReachBottom事件由mixin.js發出，目的是让頁面到底時，保证所有圖片都进行加載，做到绝對稳定且可靠
            this.$nextTick(() => {
                uni.$once('uOnReachBottom', () => {
                    if (!this.isShow) this.isShow = true;
                });
            })
            // mounted的時候，不一定挂載了這個元素，延時30ms，否则會報错或者不報错，但是也没有效果
           
            setTimeout(() => {
                // #ifndef APP-NVUE
                // 這里是组件内獲取布局狀態，不能用uni.createIntersectionObserver，而必須用this.createIntersectionObserver
                // this.disconnectObserver('contentObserver');
                // nvue 里不支持
                const contentObserver = uni.createIntersectionObserver(this);
                // 要理解這里怎么計算的，請看這個：
                // https://blog.csdn.net/qq_25324335/article/details/83687695
                contentObserver.relativeToViewport({
                    bottom: this.getThreshold,
                }).observe('.u-lazy-item-' + this.elIndex, (res) => {
                    // console.log('relativeToViewport', res)
                    if (res.intersectionRatio > 0) {
                        // 懒加載狀態改變
                        this.isShow = true;
                        // 如果圖片已經加載，去掉監聽，减少性能的消耗
                        this.disconnectObserver('contentObserver');
                    }
                })
                this.contentObserver = contentObserver;
                // #endif
                // #ifdef APP-NVUE
                this.isShow = true;
                // #endif
            }, 30)
        }
    }
</script>

<style scoped lang="scss">
    @import "../../libs/css/components.scss";

    .u-wrap {
        background-color: #eee;
        overflow: hidden;
    }

    .u-lazy-item {
        // 骗系统開啟硬件加速
        transform: transition3d(0, 0, 0);
        /* #ifndef APP-NVUE */
        // 防止圖片加載“闪一下”
        will-change: transform;
        display: block;
        width: 100%;
        /* #endif */
    }
</style>