...
export const props = defineMixin({
	props: {
		...
		// 設置鍵盘右下角按鈕的文字，仅微信小程序，App-vue和H5有效
		confirmType: {
			type: String,
		...
		// 如果textarea是在一个position:fixed的區域，需要显示指定屬性fixed為true
		fixed: {
			type: Boolean,
		...
		// 光標起始位置，自动聚焦时有效，需與selection-end搭配使用
		selectionStart: {
			type: Number,
			default: () => defProps.textarea.selectionStart
		},
		// 光標结束位置，自动聚焦时有效，需與selection-start搭配使用
		selectionEnd: {
			type: Number,
