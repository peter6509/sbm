	...
	>
		...
		>
			<block v-if="mode === 'spinner'">
				...
				<!-- #endif -->
				<!-- #ifdef APP-NVUE -->
				<!-- 此组件内部圖標部分無法設置寬高，即使通過width和height配置了也無效 -->
				<loading-indicator
					v-if="!webviewHide"
...
<script>
	 ...
	 * @property {String}			color			動画活動區域的顏色，只對 mode = flower 模式有效（默認color['u-tips-color']）
	 * @property {String}			textColor		提示文本的顏色（默認color['u-tips-color']）
	 * @property {Boolean}			vertical		文字和圖標是否垂直排列 (默認 false )
	 * @property {String}			mode			模式選擇，見官网說明（默認 'circle' ）
	 * @property {String | Number}	size			加載圖標的大小，單位px （默認 24 ）
	 * @property {String | Number}	textSize		文字大小（默認 15 ）
	 * @property {String | Number}	text			文字内容 
	 * @property {String}			timingFunction	動画模式 （默認 'ease-in-out' ）
	 * @property {String | Number}	duration		動画執行周期時間（默認 1200）
	 * @property {String}			inactiveColor	mode=circle時的暗邊顏色 
	...
	export default {
		...
		data() {
			return {
				// Array.form可以通過一個伪數组對象創建指定長度的數组
				// https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Array/from
				array12: Array.from({
					length: 12
				}),
				// 這里需要設置默認值為360，否则在安卓nvue上，會延迟一個duration周期后才執行
				// 在iOS nvue上，则會一開始默認執行两個周期的動画
				aniAngel: 360, // 動画旋轉角度
				webviewHide: false, // 監聽webview的狀態，如果隐藏了頁面，则停止動画，以免性能消耗
				loading: false, // 是否運行中，针對nvue使用
			}
		},
		computed: {
			// 當為circle類型時，给其另外三邊設置一個更轻一些的顏色
			// 之所以需要這么做的原因是，比如父组件傳了color為红色，那么需要另外的三個邊為浅红色
			// 而不能是固定的某一個其他顏色(因為這個固定的顏色可能浅蓝，导致效果没有那么细腻良好)
			otherBorderColor() {
		...
		watch: {
			show(n) {
				// nvue中，show為true，且為非loading狀態，就重新執行動画模块
				// #ifdef APP-NVUE
				if (n && !this.loading) {
					setTimeout(() => {
						this.startAnimate()
		...
		methods: {
			...
			addEventListenerToWebview() {
				...
				// 監聽webview的顯示與隐藏，從而停止或者開始動画(為了性能)
			...
			// #ifdef APP-NVUE
			nvueAnimate() {
				// nvue下，非spinner類型時才需要旋轉，因為nvue的spinner類型，使用了weex的
				// loading-indicator组件，自带旋轉功能
			...
			startAnimate() {
				...
				animation.transition(ani, {
					...
					// 每次增加360deg，為了让其重新旋轉一周
					this.aniAngel += 360
					// 動画结束后，继续循环執行動画，需要同時判斷webviewHide變量
					// nvue安卓，頁面隐藏后依然會继续執行startAnimate方法
