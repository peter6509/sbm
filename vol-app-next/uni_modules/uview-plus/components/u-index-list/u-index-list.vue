...
<script>
	 ...
	 * @property {String}			inactiveColor	右邊锚點非激活的顏色 ( 默認 '#606266' )
	 * @property {String}			activeColor		右邊锚點激活的顏色 ( 默認 '#5677fc' )
	 * @property {Array}			indexList		索引字符列表，數组形式
	 * @property {Boolean}			sticky			是否開啟锚點自動吸顶 ( 默認 true )
	 * @property {String | Number}	customNavHeight	自定義导航栏的高度 ( 默認 0 )
	 * */ 
	export default {
		name: 'u-index-list',
		mixins: [mpMixin, mixin, props],
		// #ifdef MP-WEIXIN
		// 將自定義節點設置成虚拟的，更加接近Vue组件的表現，能更好的使用flex屬性
		options: {
			virtualHost: true
		},
		// #endif
		data() {
			return {
				...
				// 設置字母指示器的高度，后面為了让指示器跟随字母，並將尖角部分指向字母的中部，需要依赖此值
				indicatorHeight: 50,
				// 字母放大指示器的top值，為了让其指向當前激活的字母
				// indicatorTop: 0
				// 當前是否正在被触摸狀態
				touching: false,
				// 滾動條顶部top值
		...
		computed: {
			// 如果有傳入外部的indexList锚點數组则使用，否则使用内部生成A-Z字母
			uIndexList() {
		...
		methods: {
			...
			init() {
				// 設置列表的高度為整個屏幕的高度
				//减去this.customNavHeight，並將this.scrollViewHeight設置為maxHeight
				//解决當u-index-list组件放在tabbar頁面時,scroll-view内容较少時，還能滾動
			...
			touchStart(e) {
				...
				// 根據當前触摸點的坐標，獲取當前触摸的為第几個字母
				// #ifdef APP-NVUE
				// 使用screenY要减去导航栏44和狀態栏24高度
			...
			touchMove(e) {
				...
				// 滑動结束后迅速開始第二次滑動時候 touching 為 false 造成不顯示 indicator 問題
				if (!this.touching) {
					this.touching = true
				}
				const {
			...
			// 触摸结束
			touchEnd(e) {
				// 延時一定時間后再隐藏指示器，為了让用户看的更直觀，同時也是為了消除快速切換u-transition的show带来的影响
				sleep(300).then(() => {
					this.touching = false
				})
			},
			// 獲取索引列表的尺寸以及單個字符的尺寸信息
			...
			setIndexListLetterInfo() {
				this.getIndexListLetterRect().then(size => {
					...
					// 消除各端导航栏非原生和原生导致的差異，让索引列表字母對屏幕垂直居中
					if (this.customNavHeight == 0) {
						// #ifdef H5
						customNavHeight = sysData.windowTop
						// #endif
						// #ifndef H5
						// 在非H5中，為原生导航栏，其高度不算在windowHeight内，這里設置為负值，后面相加時變成减去其高度的一半
					...
					this.getIndexListScrollViewRect().then(sizeScroll => {
						...
						this.letterInfo = {
							height,
							// 為了让字母列表對屏幕绝對居中，让其對导航栏进行修正，也即往上偏移导航栏的一半高度
			...
			getIndexListLetter(pageY) {
				...
				// 對H5的pageY进行修正，這是由于uni-app自作多情在H5中將触摸點的坐標跟H5的导航栏结合导致的問題
				// #ifdef H5
				// pageY += sys().windowTop
				// #endif
				// 對第一和最后一個字母做邊界處理，因為用户可能在字母列表上触摸到两端的尽頭后依然继续滑動
				...
				if (pageY < top) {
					...
					// 將触摸點的Y軸偏移值，减去索引字母的top值，除以每個字母的高度，即可得到當前触摸點落在哪個字母上
			...
			async setValueForTouch(currentIndex) {
				// 如果偏移量太小，前后得出的會是同一個索引字母，為了防抖，进行返回
				if (currentIndex === this.activeIndex) return
				this.activeIndex = currentIndex
				// #ifndef APP-NVUE || MP-WEIXIN
				// 在非nvue中，由于anchor和item都在u-index-item中，所以需要對index-item进行偏移
				this.scrollIntoView = `u-index-item-${this.uIndexList[currentIndex].charCodeAt(0)}`
				// #endif

				// #ifdef MP-WEIXIN
				// 微信小程序下，scroll-view的scroll-into-view屬性無法對slot中的内容的id生效，只能通過設置scrollTop的形式去移動滾動條
				const customNavHeight = this.customNavHeight

				// 獲取header slot的尺寸信息
				const header = await this.getHeaderRect()
				// item的top值，在nvue下，模拟出的anchor的top，類似非nvue下的index-item的top
				let top = header.height
				// console.log(top)
				const anchors = this.anchors
				// 由于list组件無法獲取cell的top值，這里通過header slot和各個item之間的height，模拟出類似非nvue下的位置信息
				let children = this.children.map((item, index) => {
					...
					top = top + item.height
					// #ifdef APP-NVUE
					// 只有nvue下，需要將锚點的高度也累加，非nvue下锚點高度是包含在index-item中的。
				...
				// #ifdef APP-NVUE
				// 在nvue中，由于cell和header為同級元素，所以實际是需要對header(anchor)进行偏移
			...
			getHeaderRect() {
				// 獲取header slot的高度，因為list组件中獲取元素的尺寸是没有top值的
				return new Promise(resolve => {
