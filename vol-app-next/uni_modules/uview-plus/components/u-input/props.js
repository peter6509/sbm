...
export const props = defineMixin({
	props: {
		...
		// #endif
		// number-數字输入鍵盘，app-vue下可以输入浮点數，app-nvue和小程序平台下只能输入整數
		// idcard-身份证输入鍵盘，微信、支付宝、百度、QQ小程序
		// digit-带小數点的數字鍵盘，App的nvue页面、微信、支付宝、百度、头条、QQ小程序
		// text-文本输入鍵盘
		type: {
			type: String,
			default: () => defProps.input.type
		},
		// 如果 textarea 是在一个 position:fixed 的區域，需要显示指定屬性 fixed 為 true，
		...
		// 指定placeholder的樣式类，注意页面或组件的style中写了scoped时，需要在类名前写/deep/
		placeholderClass: {
			type: String,
		...
		// 是否显示输入字數统计，只在 type ="text"或type ="textarea"时有效
		showWordLimit: {
			type: Boolean,
			default: () => defProps.input.showWordLimit
		},
		// 設置右下角按鈕的文字，有效值：send|search|next|go|done，兼容性详见uni-app文档
		...
		// 点击鍵盘右下角按鈕时是否保持鍵盘不收起，H5無效
		confirmHold: {
			type: Boolean,
			default: () => defProps.input.confirmHold
		},
		// focus时，点击页面的时候不收起鍵盘，微信小程序有效
		holdKeyboard: {
			type: Boolean,
			default: () => defProps.input.holdKeyboard
		},
		// 自动获取焦点
		// 在 H5 平台能否聚焦以及软鍵盘是否跟随彈出，取决于当前浏览器本身的实现。nvue 页面不支持，需使用组件的 focus()、blur() 方法控制焦点
		focus: {
			type: Boolean,
			default: () => defProps.input.focus
		},
		// 鍵盘收起时，是否自动失去焦点，目前仅App3.0.0+有效
		autoBlur: {
			type: Boolean,
		...
		// 光標起始位置，自动聚集时有效，需與selection-end搭配使用
		selectionStart: {
			type: [String, Number],
			default: () => defProps.input.selectionStart
		},
		// 光標结束位置，自动聚集时有效，需與selection-start搭配使用
		selectionEnd: {
			type: [String, Number],
		...
		// 输入框内容对齊方式，可選值為：left|center|right
		inputAlign: {
			type: String,
		...
		// 边框类型，surround-四周边框，bottom-底部边框，none-無边框
		border: {
			type: String,
			default: () => defProps.input.border
		},
		// 是否只讀，與disabled不同之处在于disabled会置灰组件，而readonly则不会
		readonly: {
			type: Boolean,
