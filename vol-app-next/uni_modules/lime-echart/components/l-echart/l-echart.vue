 ...
 * @property {String} customStyle 自定義樣式
 * @property {String} type 指定 canvas 類型
 * @value 2d 使用canvas 2d，部分小程序支持
 * @value '' 使用原生canvas，會有層級問題
 * @value bottom right	不缩放圖片，只顯示圖片的右下邊區域
 * @property {Boolean} isDisableScroll	 
 * @property {number} beforeDelay = [30]  延迟初始化 (毫秒)
 * @property {Boolean} enableHover PC端使用鼠標悬浮

 * @event {Function} finished 加載完成触發
 */
export default {
	name: 'lime-echart',
	props: {
	...
	methods: {
		...
		setChart(callback) {
			if(!this.chart) {
				console.warn(`组件還未初始化，請先使用 init`)
				return
			}
		...
		async init(echarts, ...args) {
			...
			if(args && args.length == 0 && !echarts) {
				console.error('缺少参數：init(echarts, theme?:string, opts?: object, callback?: function)')
				return
			}
			// #endif
