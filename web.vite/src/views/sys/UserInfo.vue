<template>
  <input
    ...
    :multiple="false"
  />
  <VolBox :width="400" v-model="modifyOptions.model" title="修改密碼">
    <vol-form
      ref="pwdFormRef"
    ...
    <div class="user-content">
      <div class="left">
        ...
        <div class="text">
          ...
          <p>
            <el-button plain @click="modifyPwd" size="small">修改密碼</el-button>
...
const modifyOptions = reactive({
  ...
  data: [
    [{ required: true, title: "舊密碼", type: "password", field: "oldPwd" }],
    [{ type: "password", required: true, title: "新密碼", field: "newPwd" }],
    [{ type: "password", required: true, title: "確認密碼", field: "newPwd1" }],
  ],
});
...
const savePwd = () => {
  ...
  if (modifyOptions.fields.newPwd != modifyOptions.fields.newPwd1) {
    return proxy.$message.error("兩次密碼不一致");
  }
