<template>
  ...
  <div class="layout-container" v-if="isCreated"
    ...
    <div class="view-container">
      <div class="grid-search">
        <div ref="customSearchRef">
          <vol-custom-search @customFilterClick="onCustomFilterClick" @customFilterChange="onCustomFilterChange"
            v-if="showCustom" show-filter :cache-key="table.name" ref="customSearch" :options="columns">
          </vol-custom-search>
        </div>
        <div ref="fixedSearchBox" :class="[fixedSearchForm ? 'fiexd-search-box' : 'search-box']" v-show="searchBoxShow">
          <vol-form v-if="searchFormOptions.length" ref="searchForm" :load-key="false" :label-width="labelWidth"
            :formRules="searchFormOptions" :formFields="searchFormFields" :label-position="labelPosition"
        ...
        <div class="view-header">
          ...
          <div class="search-line" v-if="!fixedSearchForm && !searchBoxShow">
            <QuickSearch ref="quickSearch" v-if="singleSearch" :searchFormOptions="searchFormOptions"
              :searchFormFields="searchFormFields" :select2Count="select2Count" :label-width="labelWidth"
              :queryFields="queryFields" :tiggerPress="search"></QuickSearch>
          </div>
          ...
          <div class="btn-group">
            <template :key="bIndex" v-for="(btn, bIndex) in gridButtons">
              ...
              <view-grid-expand :render="btn.render" :item="btn" v-else-if="btn.render"></view-grid-expand>
              <el-button v-else :type="btn.type" size="small" :color="btn.color" :dark="false" :class="btn.class"
                :plain="btn.plain" v-show="!btn.hidden" :disabled="btn.readonly || btn.disabled"
            ...
            <el-button class="setting-btn" type="default"
              style="padding-left: 8px !important; padding-right: 8px !important" size="small" :plain="true"
              v-if="showCustomSearch" @click="$refs.customSearch.show()">
              <img style="height: 15px" src="./filter.svg" alt="filter" />
            </el-button>

            <el-button class="setting-btn" type="default"
              style="padding-left: 8px !important; padding-right: 8px !important" size="small" :plain="true"
              color="#626aef" v-if="showCustom" @click="customColumClick">
      ...
      <div class="grid-container">
        ...
        <vol-table ref="table" :single="single" :rowKey="rowKey" :loadTreeChildren="loadTreeTableChildren"
          ...
          :pagination-hide="paginationHide" :url="url" :load-key="false" :defaultLoadPage="load" :index="true"
          :beginEdit="tableBeginEdit" :endEditBefore="tableEndEditBefore" :column-index="columnIndex"
          :text-inline="textInline" :ck="ck" :select2Count="select2Count" :selectable="onSelectable" :lazy="lazy"
          :defaultExpandAll="defaultExpandAll" :rowParentField="rowParentField" :expandRowKeys="expandRowKeys"
          :dragPosition="dragPosition" :spanMethod="onSpanMethod" :reserveSelection="reserveSelection"
          :sortable="sortable" @onSortEnd="onSortEnd" :extraHeight="extraHeight"></vol-table>
      </div>
    </div>
  ...
  <!-- 編輯彈出框 -->
  <vol-box v-model="boxModel" :title="boxOptions.title" :width="boxOptions.width" :height="boxOptions.height"
    :modal="boxOptions.modal" :draggable="boxOptions.draggable" :padding="0" :on-model-close="onGridModelClose"
            ...
            <div class="toolbar">
              ...
              <div class="btns detail-btns" v-show="!isBoxAudit">
                <template v-for="(btn, bIndex) in detailOptions.buttons" :key="bIndex">
                  ...
                  <el-button v-else :plain="btn.plain" v-show="!(typeof btn.hidden == 'boolean' && btn.hidden)"
                    @click="registerClick(btn.onClick)" size="small"><span :style="{ color: btn.color }"><i
                        :class="btn.icon"></i>{{ $ts(btn.name) }}</span></el-button>
                </template>
              </div>
            </div>
            <vol-table ref="detail" @loadBefore="loadDetailTableBefore" @loadAfter="loadDetailTableAfter"
              @rowChange="detailRowOnChange" @rowClick="detailRowOnClick" :url="detailOptions.url" :load-key="true"
              ...
              :endEditAfter="detailOptions.endEditAfter" :column-index="detailOptions.columnIndex"
              :ck="detailOptions.ck" :text-inline="detailOptions.textInline" :select2Count="select2Count"
              :selectable="detailOnSelectable" :spanMethod="detailSpanMethod" :sortable="detailOptions.sortable"
              @onSortEnd="detailOnSortEnd"></vol-table>
          </div>

          <div class="details" :class="{ 'details-horizontal': multiple.horizontal }">
            <!-- 多表明细2023.07.02 -->
            <detail-table ref="detailsRef" :class="{ 'detail-item-horizontal': multiple.horizontal }" :style="{
              width: multiple.horizontal ? multiple.leftWidth + 'px' : null,
              flex: multiple.leftWidth ? '' : 1,
            }" @loadBefore="loadDetailTableBefore" @loadAfter="loadDetailTableAfter" @rowChange="detailRowOnChange"
              @rowClick="detailRowOnClick" @tabsClick="detailTabsOnClick" v-if="details.length" :main-table="table.url"
              :height="detailHeight" :data="details" @onSortEnd="detailOnSortEnd" :selectable="detailOnSelectable"
              :showTabs="showTabs">
            ...
            <!-- 三级明细2023.09.17 -->
            <detail-table :class="{ 'detail-item-horizontal': multiple.horizontal }" :style="{
              width: multiple.horizontal ? multiple.rightWidth + 'px' : null,
              flex: multiple.rightWidth ? '' : 1,
            }" ref="subDetailsRef" v-if="subDetails.length" @loadBefore="loadSubInternalDetailTableBefore"
              @loadAfter="loadSubDetailTableAfter" @rowClick="detailRowOnClick" :main-table="table.url"
              :height="multiple.horizontal ? detailHeight : 200" @tabsClick="detailTabsOnClick" :data="subDetails"
              @rowChange="detailRowOnChange" @onSortEnd="detailOnSortEnd" :selectable="detailOnSelectable">
              <template #detailsBtn="{ data }">
                <slot name="detailsBtn" :data="data"></slot>
              </template>
            </detail-table>
    ...
    <template #footer>
      <div style="display: flex;align-items: center; justify-content: right;">
        ...
        <div v-show="!isBoxAudit">
          ...
          <el-button v-for="(btn, bIndex) in boxButtons" :key="bIndex" :type="btn.type" size="small" :plain="btn.plain"
            v-show="!(typeof btn.hidden == 'boolean' && btn.hidden)"
            :disabled="btn.hasOwnProperty('disabled') && !!btn.disabled" @click="registerClick(btn.onClick)">
            <i :class="btn.icon"></i>{{ $ts(btn.name) }}
          </el-button>
  ...
  <!--導入excel功能-->
  <vol-box v-if="upload.url" v-model="upload.excel" :width="600" :lazy="true"
    ...
    <UploadExcel ref="upload_excel" @importExcelAfter="importExcelAfter" :importExcelBefore="importExcelBefore"
      :url="upload.url" :template="upload.template" :desc="importDesc">
      <slot name="importContent"></slot>
    </UploadExcel>
  </vol-box>
  <!--審批彈出框 -->
  <ViewGridAudit v-if="auditInited" @auditClick="saveAudit" @flowLoadAfter="flowLoadAfter" @signAfter="signAfter"
    :option="table" ref="auditRef">
...
//審批初始化配置
import { ViewGridAuditConfig, initDefaultFormReadonlyFields } from "./ViewGridAuditConfig.jsx";
...
import { loadTableOptions, loadDyTable, isDyPage, loadDyScript } from './ViewGirdLoadScript.js'
export default {
  components: {
    ViewGridExpand,
  ...
  setup(props, { attrs, emit, expose, slots }) {
    ...
    // console.log(props.coderTableId)
    //預覽props.priview


    const useDyPage = proxy.$global.dyPage !== false;
    //獲取有動態頁面加載、動態脚本的表
    if (useDyPage && !props.priview) {
      loadDyTable(proxy)
    }
    if (props.coderTableId || props.table.dyPage || isDyPage(proxy, props)) {
      loadTableOptions(proxy, props);
    }
    ...
    if (useDyPage) {
      //動態脚本
      loadDyScript(proxy, props, dataConfig)
    }
    ...
    //初始化配置信息
    //初始化按鈕
    initButtonsAuthFields(proxy, props, dataConfig, route, hiddenFields); //初始化彈出框與明细表格按鈕

    //初始化行内編輯
    ViewGridEditTable(proxy, props, ctx, dataConfig).initEditTable();
    //初始化字典配置
    const initDicKeys = (reset) => {
      initDicData(proxy, props, ctx, dataConfig, reset); //初始下框數據源
    };
    ...
    const initAdvancedSearch = () => {
      ...
      const btnIndex = buttons.findIndex(x => { return x.value == 'Search' && !x.hidden })
      if (btnIndex == -1) {
        return;
      }
      const advancedBtn = buttons[btnIndex + 1];
      if (!advancedBtn || advancedBtn.value != 'advanced' || advancedBtn.hidden) {
        return;
      }
    ...
    onBeforeMount(async () => {
      ...
      initDicKeys();
      initDefaultFormReadonlyFields(proxy, props, dataConfig)
