<template>
  ...
  >
    <template #header>
      <div class="sort-header">{{ $ts("自定義排序") }}</div>
    </template>
    <el-alert
      :title="$ts('拖動列名可調整表格列顯示顺序')"
      type="success"
      :show-icon="true"
      :closable="false"
    >
    ...
    <div class="view-column view-column-title">
      ...
      <div class="view-column-ck">{{ $ts("是否顯示") }}</div>
      <div class="view-column-right">
        <el-tooltip class="box-item" effect="dark" placement="top">
          <i class="el-icon-warning-outline"> {{ $ts("固定字段") }}</i>
          <template #content> {{ $ts("設置表頭字段固定到左邊或者右邊顯示") }} </template>
        </el-tooltip>
      </div>
      <div class="view-column-right">
        <el-tooltip class="box-item" effect="dark" placement="top">
          <i class="el-icon-warning-outline"> {{ $ts("文本顯示") }}</i>
          <template #content>
            {{ $ts("設置表格文本顯示位置：靠左、中、右顯示") }}
          </template>
        </el-tooltip>
      </div>
    </div>
          ...
          <div class="view-column" v-for="(column, index) in columns" :key="index">
            ...
            <div class="view-column-right">
              <el-radio-group v-model="column.fx">
                <el-radio :label="$ts('無')" value="none"> </el-radio>
    ...
    <template #footer>
      <div class="dialog-footer">
        <el-button @click="model = false" icon="Close">{{ $ts("關閉") }}</el-button>
...
const save = () => {
  if (
    ...
    proxy.$message.error(proxy.$ts("至少選擇一個【是否顯示】字段"));
    return;
  }
  let sortColumns = [];
