...
import { setFormAuditReadonly } from "./ViewGridAuditConfig.jsx";
...
const modelOpenBeforeAsync = async (proxy, props, dataConfig) => {
  ...
  dataConfig.detailOptions.buttons.forEach((x) => {
    if (x.value == "refresh" || x.value == "export") {
      x.hidden = dataConfig.currentAction.value == "Add";
    }
  });

  res = await props.modelOpenBefore(
    dataConfig.currentRow.value,
    dataConfig.currentAction.value,
    dataConfig.isCopyClick.value
  );
...
export const initBox = async (proxy, props, dataConfig) => {
  ...
  //新窗口編輯
  if (dataConfig.newTabEdit.value&&!props.priview) {
    ...
    const txt =
      ...
      "(" +
      (!dataConfig.isCopyClick.value && value
        ? proxy.$ts("編輯")
        : proxy.$ts("新建")) +
      ")";
    proxy.$tabs.open({
      ...
      query: {
        id: value,
        ...(dataConfig.isCopyClick.value ? { copy: 1 } : {}),
      },
    });
...
export const getRemoteFormDefaultKeyValue = (proxy, props, dataConfig) => {
  if (
    ...
    dataConfig.remoteKeys.value.length === 0
  )
    return;
...
export const initBoxHeightWidth = (proxy, props, ctx, dataConfig) => {
  ...
  if (detail.columns && detail.columns.length > 0) {
    ...
    if (!detailOptions.height) {
      detailOptions.height =
        clientHeight - props.editFormOptions.length * 36 - 234;
      detailOptions.height =
        detailOptions.height < 240 ? 240 : detailOptions.height;
    }
  ...
  //initExtraHeight(proxy, dataConfig) //初始化表格高度，减去自定義過滤高度與查詢條件高度
};
