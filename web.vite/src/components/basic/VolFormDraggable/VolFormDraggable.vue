<template>
  <div class="drag-container">
    ...
    <div class="drag-center">
      <div class="center-top">
        ...
        <el-button type="primary" plain @click="save"><i class="el-icon-check"> </i>保存</el-button>
        <el-button type="primary" plain @click="preview(true)"><i class="el-icon-view"> </i>預覽</el-button>
        <el-button type="primary" plain @click="download"><i class="el-icon-view"> </i>下載</el-button>
        <el-button type="primary" @click="clearItems" plain><i class="el-icon-delete"> </i>清空</el-button>
        <a style="margin-left: 15px" href="http://v2.volcore.xyz/document/api"
          target="_blank">設計器基于框架volform、voltable、volupload、volbox定制開發</a>
      </div>
      <div>
        <el-alert title="關于表單設計器" type="success" :show-icon="true" :closable="false">
          <div>
            1、表單設計器基于draggable開發,為本框架自定義頁面功能的补充,框架仍以可视化代碼生成器為核心
          </div>
          <div>
            2、支持可视化設計1對1、1對多及表單下拉框自動绑定、table自動加載數據(分頁、編輯)、自動上傳文件、富文本編輯
          </div>
        </el-alert>
      </div>
                ...
                <el-form-item :required="item.required" label-position="top" style="width: 100%"
                  ...
                  <el-col>
                    ...
                    <el-select style="width: 100%" :disabled="item.readonly" v-model="item.value"
                      v-else-if="item.type == 'select'" placeholder="請選擇">
                      <el-option v-for="item in item.data" :key="item.value" :label="item.label" :value="item.value">
                      </el-option>
                    </el-select>
                    <el-select style="width: 100%" :disabled="item.readonly" v-model="item.values" :multiple="true"
                      v-else-if="item.type == 'selectList'" placeholder="請選擇">
                      <el-option v-for="item in item.data" :key="item.value" :label="item.label" :value="item.value">
                      </el-option>
                    </el-select>
                    ...
                    " :fileInfo="item.fileInfo" :url="item.url" :img="item.type == 'img'"
                      :excel="item.type == 'excel'" :multiple="item.multiple" :max-size="item.maxSize"
                      :max-file="item.maxFile" :autoUpload="item.autoUpload">
                    </vol-upload>
                    ...
                    <vol-table v-else-if="item.type == 'table'" :url="item.url" :load-key="true" :index="item.edit"
                      :tableData="item.tableData" :columns="item.columns" :height="item.height" :pagination-hide="true"
    ...
    <div class="drag-right">
      ...
      <div class="attr" v-show="currentIndex != -1">
        ...
        <div class="attr-item" v-show="currentItem.type != 'table' && currentItem.hasOwnProperty('url')">
          <div class="text">上傳接口地址(后台接口)</div>
        ...
        <div class="attr-item" v-show="currentItem.data">
          <div class="text">數據源(下拉框绑定設置中維护)</div>

          <!-- dicList -->
          <el-select style="width: 100%" v-model="currentItem.key" @change="dicChange" placeholder="請選擇數據源字典">
            <el-option v-for="item in dicList" :key="item.key" :label="item.value" :value="item.key">
            </el-option>
          </el-select>
        </div>
        <div class="attr-item" v-show="currentItem.type != 'table'">
          <div class="text">標簽宽度</div>
        ...
        <div v-show="currentItem.type == 'table'">
          ...
          <div class="attr-item attr2">
            <el-button style="width: 100%" @click="openTableModel" type="primary">table配置</el-button>
          </div>
        </div>
      </div>
    </div>
  ...
  <vol-box v-model="model" :height="300" :width="550" :lazy="true" title="彈出框">
    ...
    <template #footer>
      <div>
        <el-button type="primary" @click="model = false"><i class="el-icon-close"></i>點擊關閉</el-button>
        <el-button @click="model = false"><i class="el-icon-close"></i>關閉</el-button>
      </div>
    </template>
  </vol-box>

  <vol-box v-model="previewModel" :height="600" :width="1300" :lazy="true" :padding="1" :close="false" title="預覽">
  ...
  <vol-box v-model="tableModel" :height="600" :width="1300" :lazy="true" :padding="0" :title="currentItem.name">
    <template #content>
      <el-alert title="關于table配置" type="info" :closable="false" show-icon>
        此處table是對框架voltable基本操作的配置,如果需要事件觸發、數據加載等更多功能，請在生成后的代碼添加需要的功能，完整配置見文檔<a
      ...
      <div class="btns">
        <div class="btns-left">
          表格默認功能按鈕：
          <el-checkbox v-for="item in currentItem.buttons" :label="item.name" :key="item.name">{{ item.name
            }}</el-checkbox>
        </div>
        <div class="btns-right">
          <el-button type="primary" @click="addRow"><i class="el-icon-plus"></i>添加字段</el-button>
          <el-button type="primary" @click="delRow"><i class="el-icon-delete"></i>删除字段</el-button>
          <el-button type="primary" @click="sortRow"><i class="el-icon-sort"></i>重新排列</el-button>
        </div>
      </div>
      <vol-table :load-key="true" :tableData="currnetTableData" :columns="currentTableOption" :height="448" ref="table"
        :index="true" :pagination-hide="true" :column-index="true" :ck="true"></vol-table>
    </template>
    <template #footer>
      <div style="text-align: center">
        <el-button @click="tableModel = false"><i class="el-icon-close"></i>關閉</el-button>
        <el-button type="primary" @click="saveConfigOptions"><i class="el-icon-check"></i>保存</el-button>
      </div>
    </template>
  </vol-box>
</template>
...
export default {
  ...
  methods: {
    ...
    sortRow() {
      ...
      this.$Message.success("列顯示顺序已重新排列,點擊預覽可查看");
    },
    saveConfigOptions() {
