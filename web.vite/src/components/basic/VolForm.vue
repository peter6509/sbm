<template>
  ...
  <el-form style="width: 100%; position: relative" :inline="true" ref="volform" @submit.prevent :model="formFields"
    :label-width="labelWidth" :label-position="labelPosition || $global.labelPosition" :rules="rules" :class="[
    ...
    <template v-for="(row, findex) in formRules" :key="findex">
      <div class="vol-form-item" style="width: 100%">
        <el-form-item v-show="!item.hidden" v-for="(item, index) in row" :prop="item.field" :key="index"
          :class="[!item.field || !item.title ? 'vol-form-hiden-field' : '', 'vol-form-item-' + (item.type || 'input')]"
          ...
          <div v-else-if="item.readonly && typeof formFields[item.field] == 'function'">
            ...
            <span v-if="item.extra" :style="item.extra.style"><i :class="item.extra.icon"></i> {{ item.extra.text
              }}</span>
          </div>
          <!-- 只讀圖片或文件  -->
          <div v-else-if="isReadonlyImgFile(item, formFields)">
            <div v-if="item.type == 'img'" class="form-imgs">
              <div class="img-item" v-for="(img, imgIndex) in formFields[item.field]" :key="imgIndex">
                <img :style="item.style" :src="getSrc(img.path, http, access_token)" @error="handleImageError"
          ...
          <div v-else :class="{ 'form-item-extra': item.extra }">
            ...
            <el-tree-select style="width: 100%" v-else-if="item.type == 'treeSelect'" v-model="formFields[item.field]"
              ...
              node-key="key" :props="{ label: 'label' }" @change="
                (val) => {
            ...
            <template v-else-if="['select', 'selectList'].indexOf(item.type) != -1">
              <el-select-v2 :disabled="item.readonly || item.disabled" v-show="!item.hidden" style="width: 100%"
                ...
                :multiple="item.type == 'select' ? false : true" :placeholder="$ts(item.placeholder || '請選擇')"
                :allow-create="item.autocomplete" :options="item.data" @change="
                  (val) => {
                    item.onChange && item.onChange(val, item.data);
                  }
                " clearable>
              ...
              <el-select :disabled="item.readonly || item.disabled" v-show="!item.hidden" style="width: 100%"
                :size="size" v-else-if="item.remote || item.url" v-model="formFields[item.field]" filterable
                :ref="item.field" remote :multiple="item.type == 'select' ? false : true"
                :placeholder="$ts(item.placeholder || '請選擇')" clearable :remote-method="(val) => {
                    remoteSearch(item, formFields, val);
                  }
                  ">
                <el-option v-for="item in item.data" :key="item.key" :label="$ts(item.value)" :value="item.key">
                </el-option>
              </el-select>
              <el-select :disabled="item.readonly || item.disabled" v-show="!item.hidden" style="width: 100%"
                :size="size" v-else :ref="item.field" v-model="formFields[item.field]"
                :filterable="item.filter === undefined ? true : false" :multiple="item.type == 'select' ? false : true"
                :placeholder="$ts(item.placeholder || '請選擇')" :allow-create="item.autocomplete"
                :filter-method="item.filterMethod" @change="
                  (val) => {
            ...
            <vol-select-box v-else-if="item.type == 'selectBox'" v-model="formFields[item.field]"
              :onSelect="item.onSelect" :textInline="item.textInline" :paginationHide="item.paginationHide"
              :pagination="item.pagination" :loadBefore="item.loadBefore" :loadAfter="item.loadAfter" :url="item.url"
              :columns="item.columns" :defaultLoadPage="item.load" :single="item.single" :height="item.height"
              :readonly="item.readonly">
            </vol-select-box>

            <vol-select-table v-else-if="item.type == 'selectTable'" v-model="formFields[item.field]"
              :onSelect="item.onSelect" :textInline="item.textInline" :paginationHide="item.paginationHide"
              :pagination="item.pagination" :loadBefore="item.loadBefore" :loadAfter="item.loadAfter"
              @onKeyPress="item.onKeyPress" :blur="item.blur" :url="item.url" :columns="item.columns"
              :defaultLoadPage="item.load" :single="item.single" :height="item.height" :width="item.selectWidth"
              :readonly="item.readonly || item.disabled" :input-readonly="item.inputReadonly" :field="item.field">
            </vol-select-table>
            <el-switch v-show="!item.hidden" v-else-if="item.type == 'switch'" v-model="formFields[item.field]"
              ...
              :active-text="$ts(item.activeText || '是')" :inactive-text="$ts(item.inactiveText || '否')"
              inactive-color="rgb(194 194 194)" :width="40" :active-value="typeof formFields[item.field] == 'boolean'
                  ? true
            ...
            <el-radio-group :disabled="item.readonly || item.disabled" v-show="!item.hidden"
              ...
              <el-radio v-for="kv in item.data" :disabled="item.readonly || kv.readonly || kv.disabled" :key="kv.key"
                :label="kv.value" :value="kv.key">{{ $ts(kv.value) }}</el-radio>
            </el-radio-group>
            ...
            <el-checkbox-group :disabled="item.readonly || item.disabled" v-show="!item.hidden"
              ...
              <el-checkbox v-for="kv in item.data" :key="kv.key" :disabled="item.readonly || kv.readonly || kv.disabled"
                :label="kv.value" :value="kv.key">{{ $ts(kv.value) }}</el-checkbox>
            </el-checkbox-group>
            ...
            ">
              <el-date-picker :size="size" :ref="item.field" :disabled="item.readonly || item.disabled"
                ...
                :disabledDate="(val) => getDateOptions(val, item)" :start-placeholder="$ts('開始時間')"
                :unlink-panels="true" :end-placeholder="$ts('结束時間')" @change="
                  (val) => {
                    dateRangeChange(val, item);
                  }
                " :value-format="getDateFormat(item)" :shortcuts="getShortcuts($ts)" range-separator="-">
              </el-date-picker>
            ...
            <div v-show="!item.hidden" style="width: 100%"
              ...
              <el-date-picker :size="size" clearable :ref="item.field" :disabled="item.readonly || item.disabled"
                style="width: 100%" v-model="formFields[item.field]" @change="item.onChange" :type="item.type"
                :placeholder="$ts(item.placeholder || item.title)" :disabledDate="(val) => getDateOptions(val, item)"
                :value-format="getDateFormat(item)">
              </el-date-picker>
            </div>
            <!-- time字段：數據庫字段要用varhcar類型
             如果使用的是date/datetime類型,需要設置表單配置的字段屬性valueFormat='YYYY-MM-DD HH:mm' -->
            <el-time-picker :ref="item.field" :size="size" v-else-if="item.type == 'time'"
              v-model="formFields[item.field]" :disabled="item.readonly || item.disabled" :placeholder="$ts('時間')"
              :value-format="item.valueFormat || 'HH:mm'" :format="item.format || 'HH:mm'" @change="item.onChange"
              style="width: 100%">
            </el-time-picker>

            <el-scrollbar style="border: 1px solid #c7d8db; border-radius: 5px" :height="item.height || 150"
            ...
            <vol-upload v-show="!item.hidden" v-else-if="isFile(item, formFields)" :desc="item.desc"
              ...
              :upload-before="item.uploadBefore" :upload-after="item.uploadAfter" :append="item.multiple"
              :imgOption="item.imgOption" :on-change="(files) => {
            ...
            <div v-else-if="item.type == 'cascader'">
              ...
              <el-cascader :size="size" :ref="item.field" clearable style="width: 100%; margin-top: -3px"
                v-model="formFields[item.field]" :disabled="item.readonly || item.disabled" :options="item.data" :props="{
                  checkStrictly: item.changeOnSelect || item.checkStrictly,
                }" @change="item.onChange">
              </el-cascader>
            </div>
            <el-rate v-else-if="item.type == 'rate'" @change="
              (val) => {
            ...
            <el-input :size="size" clearable :ref="item.field" :input-style="item.inputStyle"
              ...
              }" :placeholder="$ts(item.placeholder || item.title)" @keypress="
                ($event) => {
                  onKeyPress($event, item);
                }
              " @change="item.onKeyPress" @blur="item.blur" @focus="item.focus" />
            <el-input-number :size="size" style="width: 100%" :ref="item.field" :input-style="item.inputStyle"
              v-else-if="item.type == 'number' || item.type == 'decimal'" v-model="formFields[item.field]"
              :precision="item.precision" :min="item.min" :disabled="item.readonly || item.disabled"
              :placeholder="$ts(item.placeholder || item.title)" :max="item.max" controls-position="right" @keypress="
                ($event) => {
                  onKeyPress($event, item, true);
                }
              " @change="item.onKeyPress" @blur="item.blur" @focus="item.focus" @keyup.delete="
                ($event) => {
            ...
            <el-input :size="size" clearable :input-style="item.inputStyle" v-else-if="item.type == 'password'"
              ...
              v-show="!item.hidden" :placeholder="$ts(item.placeholder || item.title)" />
            <!-- 2021.11.18修複el-input没有默認enter事件時回车異常 -->
            <el-input :size="size" clearable :ref="item.field" :input-style="item.inputStyle"
              v-else-if="item.onKeyPress" :placeholder="$ts(item.placeholder || item.title)"
              :disabled="item.readonly || item.disabled" v-show="!item.hidden" v-model="formFields[item.field]"
              @keypress="
                ($event) => {
                  onKeyPress($event, item);
                }
              " @keyup.delete.native="item.onKeyPress" @change="item.onKeyPress" @keyup.enter="item.onKeyPress"
              @blur="item.blur" @focus="item.focus"></el-input>
            <el-input :size="size" clearable v-else :ref="item.field" :input-style="item.inputStyle"
              :placeholder="$ts(item.placeholder || item.title)" :disabled="item.readonly || item.disabled"
...
const validate = async (callback) => {
  ...
  await volform.value.validate((valid) => {
    if (!valid) {
      proxy.$message.error({ message: proxy.$ts("數據驗証未通過!"), duration: 2000 });
      result = false;
      return;
    }
    if (typeof callback === "function") {
      try {
        ...
        let msg = `表單驗証回調方法異常：${error.message}`;
