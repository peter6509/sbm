...
export const initDrag = (props, dragRef, tableRef, showDragMask, realHeight) => {
  ...
  dragRef.onmousedown = (ev) => {
    ...
    ev = ev || window.event
    disY = ev.clientY
    let disH = tableRef.offsetHeight // 獲取拖拽前div的高
...
export const validateRow = (proxy, props, row, column) => {
  ...
  if (!validateColum(proxy, props, column, row)) {
    ...
    proxy.$message.error(`[${proxy.$ts(column.title)}]` + proxy.$ts(proxy._errMsg || '是必填項目'))
    return false
  }
  proxy.errField = ''
  return true
...
export const delTableRow = (proxy, edit, tableData, rows, indexArr) => {
  if (rows.length == 0) return proxy.$message.error(proxy.$ts('請選擇要删除的行!'))

  let data = tableData
  // let indexArr = this.getSelectedIndex()
  // if (indexArr.length == 0) {
  //   return proxy.$message.error('删除操作必須設置的屬性<vol-table index></vol-table>')
  // }
