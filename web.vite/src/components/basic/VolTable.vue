<template>
  <div class="vol-table" ref="refTable" :class="[
    ...
    <!-- :key="randomTableKey" -->
    <el-table :show-summary="summaryData.length > 0" :summary-method="getSummaryData" :row-key="rowKey" :lazy="lazy"
      :defaultExpandAll="defaultExpandAll" :expand-row-keys="rowKey ? expandRowKeys : undefined" stripe
      ...
      tooltip-effect="dark" :height="realHeight - extraHeight || null" :max-height="realMaxHeight"
      :data="url ? rowData : tableData" :border="true" :row-class-name="initIndex" :cell-style="getCellStyle"
      :cell-class-name="getCellClass" style="width: 100%" :scrollbar-always-on="true" @expand-change="expandChange"
      :span-method="cellSpanMethod">
      <el-table-column v-if="ck" type="selection" :fixed="fixed" :align="'center'" :selectable="selectable"
        width="55"></el-table-column>
      <!-- type="index" -->
      <el-table-column v-if="columnIndex" :fixed="fixed" width="55" :align="'center'" :label="$ts('序號')">
        <template #default="scope">
          {{ scope.$index + 1 + (paginations.page - 1) * paginations.size }}
        </template>
      </el-table-column>
      <!-- 2020.10.10移除table第一行强制排序 -->
      <el-table-column v-for="(column, cindex) in tableColumns" :prop="column.field" :label="column.title"
        ...
        :class-name="column.class" :filters="column.filterData ? getFilters(column) : undefined"
        :filter-method="column.filterData ? filterHandler : undefined">
        <template #filter-icon><el-icon>
            <Filter />
          </el-icon></template>
        <template #header>
          <table-render v-if="column.renderHeader" :column="column" :render="column.renderHeader"></table-render>
          <template v-else>
            <span v-if="(column.require || column.required) && column.edit" class="column-required">*</span><span
              :style="column.titleStyle">{{ $ts(column.title) }}</span>

            <el-tooltip placement="top" v-if="column.tip">
              <template #content>
                <div v-html="column.tip.text"></div>
              </template>
              <i :style="{ color: column.tip.color || '#7d7979' }" @click="column.tip.click"
                :class="column.tip.icon || 'el-icon-warning-outline'"></i>
            </el-tooltip>
          </template>
        </template>
        ...
        <template #default="scope">
          ...
          <el-table-column style="border: none" v-for="columnChildren in filterChildrenColumn(column.children)"
            :key="columnChildren.field || columnChildren.title" :min-width="columnChildren.width"
            :class-name="columnChildren.class" :prop="columnChildren.field" :align="columnChildren.align"
            :sortable="columnChildren.sort ? 'custom' : false"
            :show-overflow-tooltip="columnChildren.showOverflowTooltip" :label="$ts(columnChildren.title)">
            <template #default="scopeChildren">
              <a v-if="columnChildren.link" href="javascript:void(0)" style="text-decoration: none"
          ...
          ">
            <div style="display: flex; align-items: center" @click.stop>
              <i style="padding: 3px; margin-right: 10px; color: #8f9293; cursor: pointer"
          ...
          " class="edit-el">
            <div @click.stop class="e-item">
              <div>
                ...
                <el-date-picker clearable size="default" style="width: 100%" :ref="column.field + scope.$index"
                  v-if="['date', 'datetime', 'month'].indexOf(column.edit.type) != -1" v-model="scope.row[column.field]"
                  @click.prevent @change="
                    (val) => {
                      dateChange(scope.row, column, val)
                    }
                  " :type="column.edit.type" :placeholder="$ts(column.placeholder || column.title)"
                  :disabledDate="(val) => getDateOptions(val, column)" :value-format="getDateFormat(column)"
                  :disabled="initColumnDisabled(scope.row, column)" @visible-change="dateVisibleChang">
                </el-date-picker>
                <!-- time字段：數據庫字段要用varhcar類型
             如果使用的是date/datetime類型,需要設置表單配置的字段屬性edit.valueFormat='YYYY-MM-DD HH:mm' -->
                <el-time-picker clearable size="default" style="width: 100%" v-else-if="column.edit.type == 'time'"
                  v-model="scope.row[column.field]" @change="
                    (val) => {
                      column.onChange && column.onChange(scope.row, column, val)
                    }
                  " :placeholder="$ts(column.placeholder || column.title)" :value-format="column.format || 'HH:mm:ss'"
                  :disabled="initColumnDisabled(scope.row, column)">
                </el-time-picker>
                <template v-else-if="column.edit.type == 'color'">
                  {{ scope.row[column.field] }}
                  <el-color-picker @show="isDateChange = true" @hide="isDateChange = false" show-alpha
                    :teleported="true" :predefine="[
                      '#ff4500',
                      '#ff8c00',
                      '#ffd700',
                      '#90ee90',
                ...
                <el-switch v-else-if="column.edit.type == 'switch'" v-model="scope.row[column.field]"
                  active-color="#0f84ff" inactive-color="rgb(194 194 194)" :active-text="$ts(column.activeText)"
                  :inactive-text="$ts(column.inactiveText)" @change="
                    (val) => {
                ...
                <vol-select-table v-else-if="column.edit.type == 'selectTable'" v-model="scope.row[column.field]"
                  :field="column.field" :ref="column.field + scope.$index" :onSelect="(rows) => {
                    column.onSelect && column.onSelect(scope.row, rows)
                  }
                    " :textInline="column.textInline" :paginationHide="column.paginationHide"
                  :pagination="column.pagination" :loadBefore="(params, callBack) => {
                    ...
                    " :loadAfter="(rows, callBack, result) => {
                      ...
                      " :url="column.url" :columns="column.columns" :defaultLoadPage="column.load"
                  :single="column.single" :height="column.height" :input-readonly="column.inputReadonly"
                  :width="column.selectWidth">
                </vol-select-table>

                <template v-else-if="['select', 'selectList'].indexOf(column.edit.type) != -1">
                  <el-select-v2 :ref="column.field + scope.$index" style="width: 100%" size="default"
                    :props="{ label: 'value', value: 'key' }" v-if="column.bind.data.length >= select2Count"
                    v-model="scope.row[column.field]" :filterable="column.filter === undefined ? true : column.filter"
                    :multiple="column.edit.type == 'select' ? false : true"
                    :placeholder="$ts(column.placeholder || column.title)" :allow-create="column.autocomplete"
                    :options="column.bind.data" @change="column.onChange && column.onChange(scope.row, column)"
                  ...
                  <el-select :ref="column.field + scope.$index" size="default" style="width: 100%" v-else
                    v-model="scope.row[column.field]" :filterable="column.filter === undefined ? true : column.filter"
                    :multiple="column.edit.type == 'select' ? false : true"
                    :placeholder="$ts(column.placeholder || column.title)" :allow-create="column.autocomplete" @change="
                      (val) => {
                    ...
                    <el-option v-for="item in column.bind.data" :key="item.key" v-show="!item.hidden"
                      :disabled="item.disabled" :label="$ts(item.value)" :value="item.key">
                    </el-option>
                  </el-select>
                </template>
                <el-tree-select :ref="column.field + scope.$index" style="width: 100%"
                  ...
                  :show-checkbox="true" :check-strictly="true" check-on-click-node node-key="key"
                  @change="column.onChange && column.onChange(scope.row, column)" :props="{ label: 'label' }">
                  <template #default="{ data, node }"> {{ $ts(data.label) }}</template>
                </el-tree-select>
                <el-input :ref="column.field + scope.$index" v-else-if="column.edit.type == 'textarea'" type="textarea"
                  :placeholder="$ts(column.placeholder || column.title)" v-model="scope.row[column.field]"
                  :disabled="initColumnDisabled(scope.row, column)" :autosize="{
                    minRows: column.minRows || 2,
                    maxRows: column.maxRows || 10
                  }">
                </el-input>
                <el-input-number :ref="column.field + scope.$index" style="width: 100%"
                  v-else-if="column.edit.type == 'number' || column.edit.type == 'decimal'"
                  v-model="scope.row[column.field]" :precision="column.edit.type == 'number' ? 0 : column.precision"
                  :min="column.min" :disabled="column.readonly || column.disabled" :max="column.max"
                  controls-position="right" @focus="onFocus(scope.row, column, $event)"
                  @blur="onBlur(scope.row, column, $event)" @keyup.delete="inputKeypress(scope.row, column, $event)"
                  @change="inputKeypress(scope.row, column, $event)" @keypress="
                    ($event) => {
                      inputKeypress(scope.row, column, $event)
                    }
                  " />
                <input :ref="column.field + scope.$index" class="table-input"
                  v-else-if="!column.summary && !column.onKeyPress" v-model.lazy="scope.row[column.field]"
                  :placeholder="$ts(column.placeholder || column.title)"
                  :disabled="initColumnDisabled(scope.row, column)" @input="inputKeypress(scope.row, column, $event)"
                  @focus="onFocus(scope.row, column, $event)" @blur="onBlur(scope.row, column, $event)" />
                <el-input v-else :ref="column.field + scope.$index" @change="inputKeypress(scope.row, column, $event)"
                  @input="inputKeypress(scope.row, column, $event)"
                  @keyup.enter="inputKeypress(scope.row, column, $event)" size="default"
                  v-model="scope.row[column.field]" :placeholder="$ts(column.placeholder || column.title)"
                  :disabled="initColumnDisabled(scope.row, column)"
                  @blur="onBlur(scope.row, column, $event)"></el-input>
              </div>
              <div class="extra" v-if="column.extra && edit.rowIndex == scope.$index">
                <a :style="column.extra.style" style="text-decoration: none"
          ...
          <table-render v-else-if="column.render && typeof column.render == 'function'" :row="scope.row" key="rd-01"
            :index="scope.$index" :column="column" :render="column.render"></table-render>
          <!--没有編輯功能的直接渲染標簽-->
          <!-- v-text="scope.row[column.field]" -->
          <template v-else>
            <a v-if="column.link" href="javascript:void(0)" style="text-decoration: none"
              @click="link(scope.row, column, $event)" v-text="formatter(scope.row, column, true)"></a>
            <img v-else-if="column.type == 'img'"
              v-for="(file, imgIndex) in getFilePath(scope.row[column.field], column)" :key="imgIndex"
              @error="handleImageError" @click="viewImg(scope.row, column, file.path, $event, imgIndex)"
              class="table-img" :style="{
            ...
            <div v-else-if="column.formatter" @click.stop="formatterClick(scope.row, column, $event)"
            ...
            " v-else-if="column.bind">
              <el-tag v-if="useTag && column.type != 'cascader'" class="cell-tag"
                :class="[isEmptyTag(scope.row, column)]" :type="getColor(scope.row, column)" :effect="column.effect">{{
      ...
      <div class="block pagination" key="pagination-01">
        ...
        <div class="pagination-content">
          <el-pagination key="pagination-02" @size-change="handleSizeChange" @current-change="handleCurrentChange"
            ...
            :pager-count="pagerCount" layout="total, sizes, prev, pager, next, jumper"
...
const edit = reactive({ columnIndex: -1, rowIndex: -1 }) // 當前雙擊編輯的行與列坐標
// const summary = ref(false); // 是否顯示合計
// 目前只支持從后台返回的summaryData數據
const summaryData = reactive([])
const summaryIndex = reactive({})
const cellStyleColumns = reactive({})
const remoteColumns = reactive([]) // 需要每次刷新或分頁后從后台加載字典數據源的列配置
//-table带數據源的單元格是否啟用tag標簽(下拉框等單元格以tag標簽顯示)
...
// 没有定義高度與最大高度，使用table默認值 // 定義了最大高度則不使用高度
realHeight.value =
...
const dowloadFile = (file, column, index) => {

  //file,column,fIndex
  if (column && column.fileClick && column.fileClick(index, file, [file]) === false) {
    return
  }
...
const load = async (query, isResetPage) => {
  ...
  // data.summary = null;
  //没有返回合計，但有合計字段的，前端默認對當前頁面的數據進行合計處理
