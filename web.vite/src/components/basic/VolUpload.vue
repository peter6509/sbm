    ...
    <div>
      ...
      <div class="input-btns" style="display: flex">
        ...
        <el-button v-if="!autoUpload && !img" type="info" :disabled="changed" @click="upload(true)"
          :loading="loadingStatus">{{ $ts('上傳文件') }}</el-button>
      </div>
      <slot></slot>
      <div v-if="desc">
        <el-alert :title="getText() + '文件大小不超過' + (maxSize || 50) + 'M'" type="info" show-icon>
        </el-alert>
      </div>
...
export default {
  ...
  props: {
    ...
    fileInfo: {
      //用于接收上傳的文件，也可以加以默認值，顯示已上傳的文件，用户上傳后會覆盖默認值
      type: Array,
      default: () => {
        return []
    ...
    img: {
      //圖片類型  img>excel>fileTypes三種文件類型优先级
      type: Boolean,
      default: false
    },
    excel: {
  ...
  data() {
    return {
      ...
      changed: false, //手動上傳成功后禁止重複上傳，必須重新選擇
      model: true,
      files: [],
      bigImg: '',
  ...
  methods: {
    ...
    convertToFile(dataurl, filename) {
      ...
      // new File返回File對象 第一個参數是 ArraryBuffer 或 Bolb 或Arrary 第二個参數是文件名
      // 第三個参數是 要放到文件中的内容的 MIME 類型
    ...
    async uploadOSS() {
      this.http.get('api/alioss/getAccessToken', {}, false).then(async (x) => {
        ...
        let client = new OSS({
          ...
          // 從STS服務獲取的临時訪問密鑰（AccessKey ID和AccessKey Secret）。
    ...
    async upload(vail) {
      ...
      //增加上傳時自定義参數，后台使用獲取Utilities.HttpContext.Current.Request.Query["字段"]
      let params = {}
    ...
    checkFile(inputFiles) {
      ...
      if (this.multiple && files.length + (inputFiles || []).length > (this.maxFile || 5)) {
        this.$message.error(
          '最多只能選【' + (this.maxFile || 5) + '】' + (this.img ? '張圖片' : '個文件') + ''
        )
        return false
      }
      if (!inputFiles) {
      ...
      for (let index = 0; index < inputFiles.length; index++) {
        ...
        if (this.fileTypes && this.fileTypes.length > 0 && !this.format(file, this.fileTypes)) {
          this.$message.error(
            '選擇的文件【' + file.name + '】只能是【' + this.fileTypes.join(',') + '】格式'
          )
          return false
        }
