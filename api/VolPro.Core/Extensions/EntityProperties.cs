...
namespace VolPro.Core.Extensions
{
    public static class EntityProperties
    {
        ...
        /// <param name="lenght">是否包括后字段具體长度:nvarchar(100)</param>
        /// <returns></returns>
        public static KeyValuePair<string, string> GetColumnType(this PropertyInfo property, bool lenght = false)
        {
            ...
            if (asType)
            {
                ...
                if (!string.IsNullOrEmpty(colType))
                {
                    ...
                    ///如果是string,根據 varchar或nvarchar判断最大长度
        ...
        /// 
        /// </summary>
        /// <param name="array">將數组转換成sql语句</param>
        ...
        /// <summary>
        ///<param name="sql">要執行的sql语句如：通過EntityToSqlTempName.Temp_Insert0.ToString()字符串占位，生成的的sql语句會把EntityToSqlTempName.Temp_Insert0.ToString()替換成生成的sql临時表數據
        ...
        /// <param name="fieldType">指定生成的數组值的類型</param>
        /// <param name="sql"></param>
        /// <returns></returns>
                  ...
                  )
        {
            ...
            {
                throw new Exception("写入數據太多,請分開写入。");
            }
            ...
            {
                //每1000行需要分批写入(數據庫限制每批至多写入1000行數據)
        ...
        /// <summary>
        ///此方法适用于數據量少,只有几列數據，不超過1W行，或几十列數據不超過1000行的情况下使用
        /// 大批量的數據考虑其他方式
        ...
        /// <param name="keyType">true获取key對應類型,false返回對象Key的名稱</param>
        /// <returns></returns>
        public static string GetKeyName(this PropertyInfo[] properties, bool keyType)
        {
            ...
            foreach (PropertyInfo propertyInfo in properties)
            {
                ...
                //如果没有ColumnAttribute的需要單獨再驗証，下面只驗証有屬性的
        ...
        /// <param name="expression">對指定屬性進行驗証x=>{x.Name,x.Size}</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// specificProperties=null並且validateProperties=null，對所有屬性驗証，只驗証其是否合法，不驗証是否為空(除屬性標識指定了不能為空外)
        /// specificProperties!=null，對指定屬性校驗，並且都必须有值
        /// null並且validateProperties!=null，對指定屬性校驗，不判断是否有值
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="entity"></param>
        /// <param name="specificProperties">驗証指定的屬性，並且非空判断</param>
        /// <param name="validateProperties">驗証指定屬性，只對字段合法性判断，不驗証是否為空</param>
        /// <returns></returns>
        ...
        {
            WebResponseContent responseData = new WebResponseContent();
            if (entity == null) return responseData.Error("對象不能為null");
            ...
            foreach (PropertyInfo propertyInfo in propertyArray)
            {
                ...
                //若存在specificProperties並且屬性為數组specificProperties中的值，校驗時就需要判断是否為空
        ...
        /// <summary>
        /// 获取數據庫類型，不带长度，如varchar(100),只返回的varchar
        /// </summary>
        /// <param name="propertyInfo"></param>
        /// <returns></returns>
        ...
        /// 驗証數據庫字段類型與值是否正确，
        /// </summary>
        /// <param name="propertyInfo">propertyInfo為當字段，當前字段必须有ColumnAttribute屬性,
        /// 如字段:標識為數據庫int類型[Column(TypeName="int")]  public int Id { get; set; }
        /// 如果是小數float或Decimal必须對propertyInfo字段加DisplayFormatAttribute屬性
        /// </param>
        /// <param name="value"></param>
        /// <returns>IEnumerable<(bool, string, object)> bool成否校驗成功,string校驗失败信息,object,當前校驗的值</returns>
        ...
        /// <param name="propertyInfo">要驗証的類的屬性，若不為null，则會判断字符串的长度是否正确</param>
        /// <returns>(bool, string, object)bool成否校驗成功,string校驗失败信息,object,當前校驗的值</returns>
        ...
        /// <param name="required">是否指定當前屬性必须有值</param>
        /// <returns></returns>
        ...
        {
            if (dic == null || dic.Count == 0) { return "参數无效"; }
            if (propertyInfo == null)
            ...
            foreach (PropertyInfo property in propertyInfo)
            {
                ...
                {
                    //移除主鍵默認為新增數據，將不在编輯列中的有默認值的數據設置為默認值
                    //如果為true默認為添加功能，添加操作所有不能為空的列也必须要提交
                    ...
                    {
                        return property.GetTypeCustomValue<DisplayAttribute>(x => x.Name) + "為必须提交项";
                    }
                    continue;
                }
                ...
                if (!isEdit)
                {
                    ...
                    {
                        return property.GetTypeCustomValue<DisplayAttribute>(x => x.Name) + "没有配置好Model為编輯列";
                    }
                    if (removeNotContains)
                    {
        ...
        /// <summary>
        /// 將實體映射到输出到ApiOutput實體上,如果没有输入實體，则返回null
        /// </summary>
        /// <param name="entity"></param>
        /// <returns></returns>
        ...
        /// <param name="resultExpression">只映射返回對象的指定字段,若為null则默認為全部字段</param>
        /// <param name="sourceExpression">只映射數據源對象的指定字段,若為null则默認為全部字段</param>
        /// <returns></returns>
        ...
        /// false=编輯設置"ModifyID", "Modifier", "ModifyDate"值
        /// </param>
