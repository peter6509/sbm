...
namespace VolPro.Core.Dapper
{
    public class SqlDapper : ISqlDapper
    {
        ...
        /// <param name="beginTransaction">是否開啟事務</param>
        /// <returns></returns>
        ...
        public int AddRange<T>(IEnumerable<T> entities, Expression<Func<T, object>> addFileds = null, bool beginTransaction = true)
        {
            ...
            if (key == null)
            {
                throw new Exception("實體必须包括主鍵才能批量更新");
            }
            string[] columns;
            ...
            else
            {
                ...
                //2020.11.21修复sqlserver批量写入主鍵類型判断错误
        ...
        /// <summary>
        /// sqlserver使用的临時表参數化批量更新，mysql批量更新待發開
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="entity">實體必须带主鍵</param>
        ...
        /// <summary>
        ///(根據主鍵批量更新實體) sqlserver使用的临時表参數化批量更新，mysql待优化
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="entities">實體必须带主鍵</param>
        ...
        public int UpdateRange<T>(IEnumerable<T> entities, Expression<Func<T, object>> updateFileds = null, bool beginTransaction = false)
        {
            ...
            if (DBType.Name == DbCurrentType.MySql.ToString())
            {
                ...
                // throw new Exception("mysql批量更新未實现");
            }
        ...
        public int DelWithKey<T>(bool beginTransaction = false, params object[] keys)
        {
            ...
            {
                throw new Exception($"主鍵類型【{validation.Where(x => !x.Item1).Select(s => s.Item3).FirstOrDefault()}】不正确");
            }
            ...
            catch (Exception ex)
            {
                ...
                if (ex.Message.Contains("local data is disabled"))
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        Console.WriteLine($"開啟mysql日志写入异常:{e.Message}");
                    }
                }
        ...
        {
            //以半角逗號（即,）作分隔符，列為空也要表达其存在。
            //列内容如存在半角逗號（即,）则用半角引號（即""）將该字段值包含起来。
            //列内容如存在半角引號（即"）则應替換成半角双引號（""）转義，並用半角引號（即""）將该字段值包含起来。
            ...
            foreach (DataRow row in table.Rows)
            {
                for (int i = 0; i < table.Columns.Count; i++)
                {
                    ...
                    {
                        //centos系统里把datatable里的日期转換成了10/18/18 3:26:15 PM格式
