...
namespace VolPro.Core.Quartz
{
    public static class QuartzNETExtension
    {
        ...
        public static IApplicationBuilder UseQuartz(this IApplicationBuilder applicationBuilder, IWebHostEnvironment env)
        {
            ...
            {
                Console.WriteLine($"作業啟動异常:{ex.Message + ex.StackTrace}");
            }
        ...
        public static async Task<object> AddJob(this Sys_QuartzOptions taskOptions, ISchedulerFactory schedulerFactory, IJobFactory jobFactory = null)
        {
            ...
            try
            {
                ...
                {
                    await schedulerFactory.TriggerAction(JobAction.開啟, taskOptions);
                ...
                {
                    msg = $"添加作業失败，作業:{taskOptions.TaskName},表达式不正确:{taskOptions.CronExpression}";
               ...
               .WithIdentity(taskOptions.Id.ToString(), "group").Build();
                ...
                {
                    Console.WriteLine("啟動了：" + taskOptions.TaskName);
                ...
                msg = $"作業啟動:{taskOptions.TaskName}";
                Console.WriteLine(msg);
                QuartzFileHelper.Error(msg);
            }
            catch (Exception ex)
            {
                msg = $"作業啟動异常:{taskOptions.TaskName},{ex.Message}";
                Console.WriteLine(msg);
                QuartzFileHelper.Error(msg);
        ...
        {
            return schedulerFactory.TriggerAction(JobAction.開啟, taskOptions);
            ...
            try
            {
                ...
                switch (action)
                {
                    case JobAction.删除:
                        ...
                        await scheduler.UnscheduleJob(trigger.Key);// 移除触發器
        ...
        /// 
        /// </summary>
        /// <param name="context"></param>通過作業上下文获取作業對應的配置参數
        /// <returns></returns>
        ...
        public static (bool, object) Exists(this Sys_QuartzOptions taskOptions, bool init)
        {
            if (!init && _taskList.Any(x => x.TaskName == taskOptions.TaskName && x.GroupName == taskOptions.GroupName))
            {
                return (false,
                    ...
                    {
                        status = false,
                        msg = $"作業:{taskOptions.TaskName},分组：{taskOptions.GroupName}已經存在"
                    });
            }
            return (true, null);
        }
        ...
        public static (bool, string) IsValidExpression(this string cronExpression)
        {
            try
            {
                ...
                return (date != null, date == null ? $"請确認表达式{cronExpression}是否正确!" : "");
            }
            catch (Exception e)
            {
                return (false, $"請确認表达式{cronExpression}是否正确!{e.Message}");
            }
        }
    }
